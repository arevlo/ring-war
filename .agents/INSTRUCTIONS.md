...
A turn-based capture-the-flag game between two AI agent teams, triggered by an NFC ring tap. Built on the Notion Developer Platform (Workers runtime + Notion API). Claude Sonnet 4.6 is the brain for both teams and the arbiter.

This file is the load-bearing context for every Claude Code session in this repo. Read it first. Then read the Notion docs linked below — those are the spec; this is a pointer.

---

## Canonical docs (Notion)

These pages are the spec. Always read the relevant one before touching the corresponding area. If the Notion MCP is configured for your session, fetch live; otherwise ask Carlos to paste the page.

**Reference (read these first):**

| # | Doc | URL |
|---|---|---|
| 00 | Index (workspace TOC) | https://www.notion.so/3628b6b19916818c8189d0252bb7f0f2 |
...
**Tracking DBs:**
- Ring War Index (entity → status, includes all 13 docs above as `Doc`-typed rows): https://www.notion.so/21ebd461eb8a494e9036a0dcb58afc44
- Ring War Worktrees (parallel build plan): https://www.notion.so/3a7ffd935ec94377ae91c1990ca81a5e

---

## Architecture in three sentences

A single NFC tap hits a Notion Worker webhook. The handler reads game state from Notion, makes three sequential Claude calls (Order moves → Shadow moves → Throne judges), executes only the winning team's tool against Notion, and writes a turn entry to the activity log. The dashboard (Vercel-hosted static HTML) polls a state endpoint on the Worker every two seconds.

---

## Invariants (do not violate)

- **One Worker file**: `src/index.ts`. All backend lives here.
- **Six tool registrations**: three Order (`vault_ring`, `audit_public`, `unmake_ring`) and three Shadow (`pilfer_ring`, `corrupt_vault`, `leak_whisper`). Both teams have API-level access to all six; per-team restriction is via system prompts.
- **One webhook**: `worker.webhook("tap", ...)`. Every trigger surface (NFC ring, NFC sticker, dashboard button, curl) hits the same URL.
- **Three Claude calls per tap, in order**: Order → Shadow → Throne. The Throne is called with **no tools registered** and must output strict JSON: `{ "winner": "order" | "shadow", "reasoning": "<one sentence>" }`.
- **Tools return a `StateDelta`**; the Worker handler applies the delta and writes Ring State. Tools do NOT mutate Ring State directly.
- **`max_tokens: 1024`** on every Claude call. Anthropic console usage cap set to $30. Hard caps; non-negotiable.
- **Voice fidelity**: copy from Order, Shadow, and Throne must match the system prompts in doc 02. If a voice drifts, fix the prompt — don't improvise around it.
...
src/
  ...
  tools/
    ...
    leak_whisper.ts
dashboard/
  index.html            # Static dashboard; polls /state endpoint
  vercel.json
.dev.vars.example
LICENSE                 # MIT
README.md
...
Every entity in the **Ring War Index** DB maps to a file or function in this repo. Maintain the bi-directional link:

**In code → Notion:** each source file's top-of-file header comment includes the entity's Notion URL.

```typescript
// src/tools/vault_ring.ts
//
// Notion: https://www.notion.so/3628b6b1991681c4b782c70d0bf72cba
// Order team. Pull the Ring into the Vault. holder = order, integrity += 8.
```

**In Notion → code:** each Index entry's page body includes a status line and a source path.

```
Status: done
Source: src/tools/vault_ring.ts
Last edited: 2026-05-17
```

When you implement or modify an entity:
1. Read its Index page body first — that's the contract.
2. If the spec needs to change, propose the change to Carlos before editing code.
3. After implementing, update the Index page body (Status + Source + Last edited).

The Worktrees DB is the parallel-build coordinator. If you're in a worktree, the entry there names which Index entities you own. Don't touch entities owned by another worktree.

---

## Environment

Required env vars (in `.dev.vars` for local; in Worker secrets for deploy):

| Var | Source |
|---|---|
| `ANTHROPIC_API_KEY` | Anthropic console |
| `RING_STATE_DB_ID` | Notion (Ring State DB) |
| `VAULT_DB_ID` | Notion (Vault DB) |
| `TURNS_DB_ID` | Notion (Turns DB) |
| `PUBLIC_DB_ID` | Notion (Public DB) |

`NOTION_TOKEN` is auto-injected by the Notion Workers runtime — do not set it manually.
...
```bash
ntn workers dev                 # local dev server
ntn workers deploy              # deploy to Notion
curl -X POST <worker-url>/tap   # smoke test one turn
...
Per tap with prompt caching: ~$0.04–0.08. Full 6-turn game: ~$0.40. Console cap: $30.

If you're about to write a loop that calls Claude, stop and add a max-turn guard (game caps at turn 10) and a max-tap-rate guard in the webhook handler before deploying. Bugs at 2am can't bankrupt anyone.

---

## Stretch goals (Sunday only; do not start if core is unfinished)

1. MiniMax TTS for Throne verdicts (autoplay on dashboard).
2. Tap sound on the dashboard (HTML5 audio, single file).
3. Custom Agent registration of `audit_public` for `@agent` calls inside Notion.
4. Game history page on the dashboard.

---

## Demo arc (memorize for Sunday 11am)

> Two AI agent teams fighting over a single credential. The only way to advance the game is to tap the Ring.

Three minutes. Tap five times. Show the dashboard live. End on a destroyed or exfiltrated Ring. Pivot to: one Worker file, three Claude calls, Notion is the state layer, the ring is the trigger and the prize. Done.

---

# Notion Workers SDK Reference

The remainder of this file is the generic Notion Workers SDK reference from the `ntn` scaffold. Treat as appendix — consult when implementing specific Worker capabilities (syncs, OAuth, webhook signature verification, etc.). The Ring War-specific spec above takes precedence on any conflict.
